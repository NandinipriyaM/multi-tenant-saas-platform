...
const createProject = async (req, res) => {
  try {
    ...
    if (parseInt(projectCountResult.rows[0].count) >= tenantResult.rows[0].max_projects) {
      return res.status(403).json({
        success: false,
        message: "Project limit reached for your subscription plan",
      });
 ...
 * Tenant users → tenant scoped projects
 */
const listProjects = async (req, res) => {
  try {
    const currentUser = req.user;
    ...
    let query = `
      SELECT p.id, p.name, p.description, p.status, p.created_by, p.created_at, u.full_name
      FROM projects p
      LEFT JOIN users u ON p.created_by = u.id
    `;
    ...
    if (currentUser.role !== "super_admin") {
      query += ` WHERE p.tenant_id = $${paramIndex}`;
      values.push(currentUser.tenantId);
      paramIndex++;
      whereAdded = true;
    }

    if (status) {
      query += ` ${whereAdded ? "AND" : "WHERE"} p.status = $${paramIndex}`;
      values.push(status);
      paramIndex++;
      whereAdded = true;
    }
    ...
    if (search) {
      query += ` ${whereAdded ? "AND" : "WHERE"} LOWER(p.name) LIKE LOWER($${paramIndex})`;
      values.push(`%${search}%`);
      paramIndex++;
      whereAdded = true;
    }
    ...
    if (currentUser.role !== "super_admin") {
      countQuery += ` WHERE tenant_id = $${countParamIndex}`;
      countValues.push(currentUser.tenantId);
      countParamIndex++;
      countWhereAdded = true;
    }

    if (status) {
      countQuery += ` ${countWhereAdded ? "AND" : "WHERE"} status = $${countParamIndex}`;
      countValues.push(status);
      countParamIndex++;
      countWhereAdded = true;
    }
    ...
    if (search) {
      countQuery += ` ${countWhereAdded ? "AND" : "WHERE"} LOWER(name) LIKE LOWER($${countParamIndex})`;
      countValues.push(`%${search}%`);
    }
    ...
    const projectsWithStats = await Promise.all(
      result.rows.map(async (project) => {
        ...
        return {
          ...
          taskCount: parseInt(taskStats.rows[0].total) || 0,
          completedTaskCount: parseInt(taskStats.rows[0].completed) || 0,
          createdAt: project.created_at,
        };
      })
    );
...
const updateProject = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE projects SET ${updates.join(", ")} WHERE id = $${index}
